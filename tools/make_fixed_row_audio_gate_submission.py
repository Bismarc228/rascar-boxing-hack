#!/usr/bin/env python3
"""Train fixed-row audio gate on validation rows and apply it to a submission."""
...
from tools.evaluate_fixed_row_audio_gate import combined_features, compute_audio_tracks, progress
...
def main() -> int:
    ...
    train_indexes = load_candidate_indexes(train_keys, args.train_tracks_dir, config) if args.feature_mode != "audio" else {}
    train_audio = load_audio_tracks(
        train_keys,
        train_video_by_key,
        args.data_root,
        args.sample_rate,
        args.quiet,
    ) if args.feature_mode != "pose" else {}
    x_train = np.stack(
        [
            combined_features(
                row,
                train_video_by_key[row["video_key"]],
                train_indexes.get(row["video_key"], {}),
                train_audio.get(row["video_key"]),
                args.match_window,
                pose_windows,
                audio_windows,
                audio_offsets,
    ...
    test_indexes = load_candidate_indexes(test_keys, args.test_tracks_dir, config) if args.feature_mode != "audio" else {}
    test_audio = load_audio_tracks(
        test_keys,
        test_video_by_key,
        args.data_root,
        args.sample_rate,
    ...
    x_test = np.stack(
        [
            combined_features(
                ...
                test_indexes.get(row["video_key"], {}),
                test_audio.get(row["video_key"]),
                args.match_window,
                pose_windows,
                audio_windows,
                audio_offsets,
...
def load_audio_tracks(
    ...
    sample_rate: int,
    quiet: bool,
) -> dict[str, dict[str, np.ndarray | list[int]]]:
    return {
